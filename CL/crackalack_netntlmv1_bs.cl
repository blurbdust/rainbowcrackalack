...
__constant uint BS_BASE[8] = {31, 63, 23, 55, 15, 47, 7, 39};

#define BS_CHAINS_PER_ITEM 32


__kernel void crackalack_netntlmv1_bs(
  ...
  uint key[56], out[64];
  unsigned long chal_bits = 0;   /* in[i] is all-ones iff bit i is set */
  unsigned int reduction_offset = *g_reduction_offset;
  unsigned int chain_len = *g_chain_len;
  unsigned int pos = *g_pos_start;
  unsigned long base = (unsigned long)get_global_id(0) * BS_CHAINS_PER_ITEM;

  /* ---- transpose 32 start indices into bitslice form ---- */
  for (int k = 0; k < 56; k++)
    key[k] = 0;
  for (int s = 0; s < BS_CHAINS_PER_ITEM; s++) {
    unsigned long idx = g_indices[base + s];
    uint bit = 1u << (31 - s);
    for (int k = 0; k < 56; k++)
      if ((idx >> (55 - k)) & 1UL)
        key[k] |= bit;
  ...
  for (; pos < chain_len - 1; pos++) {
    ...
    {
      unsigned long addend = (unsigned long)reduction_offset + (unsigned long)pos;
      uint carry = 0;
      for (int i = 0; i < 56; i++) {
        int b = i >> 3, j = i & 7;

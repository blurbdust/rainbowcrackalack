...
__device__ __constant__ uint BS_BASE[8] = {31, 63, 23, 55, 15, 47, 7, 39};

#define BS_CHAINS_PER_ITEM 32


extern "C" __global__ void crackalack_netntlmv1_bs(
  ...
  uint key[56], out[64];
  unsigned long long chal_bits = 0;   /* in[i] is all-ones iff bit i is set */
  unsigned int reduction_offset = *g_reduction_offset;
  unsigned int chain_len = *g_chain_len;
  unsigned int pos = *g_pos_start;
  unsigned long long base = (unsigned long long)(blockIdx.x * blockDim.x + threadIdx.x) * BS_CHAINS_PER_ITEM;

  /* ---- transpose 32 start indices into bitslice form ---- */
  for (int k = 0; k < 56; k++)
    key[k] = 0;
  for (int s = 0; s < BS_CHAINS_PER_ITEM; s++) {
    unsigned long long idx = g_indices[base + s];
    uint bit = 1u << (31 - s);
    for (int k = 0; k < 56; k++)
      if ((idx >> (55 - k)) & 1ULL)
        key[k] |= bit;
  ...
  for (int b = 0; b < 8; b++)
    for (int j = 0; j < 8; j++)
      if ((g_challenge[b] >> j) & 1)
        chal_bits |= 1ULL << (BS_BASE[j] - b);

  /* ---- walk the chains ---- */
  for (; pos < chain_len - 1; pos++) {
    for (int i = 0; i < 64; i++)
      out[i] = 0u - (uint)((chal_bits >> i) & 1ULL);
    ...
    {
      unsigned long long addend = (unsigned long long)reduction_offset + (unsigned long long)pos;
      uint carry = 0;
      for (int i = 0; i < 56; i++) {
        int b = i >> 3, j = i & 7;
        uint a = out[(BS_BASE[j] - b + 32) & 63];
        uint cmask = 0u - (uint)((addend >> i) & 1ULL);   /* 0 or ~0 */

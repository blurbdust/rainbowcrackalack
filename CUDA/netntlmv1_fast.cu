 ...
 * "*SK++ =" assignments in des_ecb_setkey (netntlmv1.cu); deriving each round's
 * subkey on the fly means the full 32-word SK[] schedule is never materialized,
 * which avoids the local-memory spill that a live SK[32] array causes. */
 ...
 * chain step and every thread (the challenge is constant for the whole launch).
 * Computing it once per thread, before the chain loop, and reusing the result
 * removes the GET_UINT32_BE + DES_IP work from every hash evaluation. */
__device__ inline void netntlmv1_challenge_to_ip(
...
/* ---- Fast hash using __shared__ S-boxes, pre-permuted challenge ----
 *
 * cx/cy are the challenge after DES_IP (see netntlmv1_challenge_to_ip).  This
 * is the per-step workhorse; callers hoist the IP out of their chain loop. */
__device__ inline void netntlmv1_hash_fast_ip(
    uint32_t SK[32],
    ...
    uint32_t *l_SB7, uint32_t *l_SB8)
{
   ...
   * Mirrors the head of des_ecb_setkey (netntlmv1.cu). */
  uint32_t c, d;
   ...
   * and apply the Feistel round immediately.  The rotation schedule and the
 ...
 * Thin wrapper that derives the IP-permuted challenge per call.  Retained for
 * callers that are not in a tight loop; loops should hoist the IP and call
 * netntlmv1_hash_fast_ip directly. */

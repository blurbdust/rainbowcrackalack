...
__device__ __constant__ uint BSP_BASE[8] = {31, 63, 23, 55, 15, 47, 7, 39};

#define BSP_SLOTS 32


extern "C" __global__ void precompute_netntlmv1_7_bs(
  ...
  unsigned long long idx[BSP_SLOTS];
  ...
  unsigned long long chal_bits = 0;
  for (int b = 0; b < 8; b++)
    for (int j = 0; j < 8; j++)
      if ((challenge_local[b] >> j) & 1)
        chal_bits |= 1ULL << (BSP_BASE[j] - b);
  ...
  for (long i = sync_pos; i < (long)chain_len - 1; i++) {
    ...
    for (int w = 0; w < 64; w++)
      out[w] = 0u - (uint)((chal_bits >> w) & 1ULL);
    ...
    unsigned long long addend = (unsigned long long)reduction_offset + (unsigned long long)i;
    uint carry = 0;
    for (int bit = 0; bit < 56; bit++) {
      int b = bit >> 3, j = bit & 7;
      uint a = out[(BSP_BASE[j] - b + 32) & 63];
      uint cmask = 0u - (uint)((addend >> bit) & 1ULL);
  ...
  for (unsigned int s = 0; s < BSP_SLOTS; s++) {
    ...
    g_output[((unsigned long long)hash_idx * (unsigned long long)output_len) + k] = result;
  }
}

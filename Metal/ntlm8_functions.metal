...
inline void index_to_plaintext_ntlm8(ulong index, constant char *charset, thread unsigned char *plaintext) {
  // divide and modulo index by 95**4
  unsigned int indexHi = (unsigned int) (index / 81450625);
  unsigned int indexLo = (unsigned int) (index - (ulong) 81450625 * indexHi);
...
inline void md4_encrypt(thread uint *hash, thread uint *W)
{
	hash[0] = 0x67452301;
	hash[1] = 0xefcdab89;
	hash[2] = 0x98badcfe;
...
inline ulong hash_ntlm8(thread unsigned char *plaintext) {
...
inline ulong hash_to_index_ntlm8(ulong hash, unsigned int pos) {
  return (hash + pos) % 6634204312890625UL;
}


inline ulong hash_char_to_index_ntlm8(device unsigned char *hash_value, unsigned int pos) {

...
constant uint BSP_BASE[8] = {31, 63, 23, 55, 15, 47, 7, 39};

#define BSP_SLOTS 32


kernel void precompute_netntlmv1_7_bs(
  ...
  for (long i = sync_pos; i < (long)chain_len - 1; i++) {
    ...
    ulong addend = (ulong)reduction_offset + (ulong)i;
    uint carry = 0;
  ...
  for (unsigned int s = 0; s < BSP_SLOTS; s++) {
    ...
    g_output[((ulong)hash_idx * (ulong)output_len) + k] = result;
  }
}
